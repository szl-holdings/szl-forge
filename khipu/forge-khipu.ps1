# SZL-Khipu-1.5B-BrainNavigator -- one-shot owner forge (paste-once bootstrap).
#
...
Say "== SZL-Khipu-1.5B-BrainNavigator : one-shot forge =="
...
$kit = Join-Path $root 'khipu'
if (-not (Test-Path (Join-Path $kit 'train_khipu.py'))) { throw "kit not found at $kit" }
Set-Location $kit
Say "    kit: $kit"
