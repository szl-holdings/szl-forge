#!/usr/bin/env python3
# SZL-Khipu-1.5B-BrainNavigator — owner-side receipt signer + keygen.
# SPDX-License-Identifier: Apache-2.0
# (c) 2026 Lutar, Stephen P. - SZL Holdings
#
# This is the trust root of the Khipu forge. It:
...
def canonical_json(value):
    ...
    Floats are REFUSED (not canonicalized): the doctrine is strings+ints only,
    because a float can round-trip differently across Python and JS and break
    byte-equality. A loss/score belongs in the payload as a STRING or an int
    count -- so make a float unrepresentable at the signer rather than emit a
    receipt that would fail server-side."""
...
def key_pem_path() -> str:
    ...
    if env:
        return env
    return os.path.join(os.path.expanduser("~"), ".a11oy", "khipu_owner_ed25519.pem")
...
def _cli() -> None:
    parser = argparse.ArgumentParser(description="SZL Khipu receipt signer")
