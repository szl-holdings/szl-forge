...
from frontier.asyncgrpo_lora_install import (
    INSTALL_TIMEOUT_SECONDS,
    JobLocalInstallError,
    prepare_job_local,
    probe_interpreter,
)
...
class ClosureRecordTests(unittest.TestCase):
    ...
    def test_probe_interpreter_on_this_host_has_no_trl_closure(self) -> None:
        probed = probe_interpreter(sys.executable)
        self.assertIs(probed["trlPresent"], False)
        self.assertIs(probed["peftPresent"], False)
        self.assertIs(probed["vllmPresent"], False)
        self.assertIs(probed["cudaAvailable"], False)

    def test_install_timeout_allows_real_vllm_wheels(self) -> None:
        self.assertGreaterEqual(INSTALL_TIMEOUT_SECONDS, 1800)

    def test_probe_interpreter_rejects_empty_path(self) -> None:
        with self.assertRaises(JobLocalInstallError):
            probe_interpreter("")
