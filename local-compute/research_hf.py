...
from inference.research_cycle import Corpus, canonical, run_cycle, sha256, strict_object
...
class HFGenerator:
    ...
    def __call__(self, messages: list[dict]) -> str:
        ...
        request = Request(ROUTER, data=canonical({"model": self.model, "messages": messages,
            "max_tokens": 1536, "temperature": 0, "stream": False,
        ...
        if not isinstance(choices, list) or len(choices) != 1 or choices[0].get("finish_reason") != "stop":
            raise ValueError("hosted completion incomplete")
...
def main() -> int:
    ...
    generator = HFGenerator(args.model, get_token() or "")
    ...
    report.update(model={"id": args.model, "serving_revision": "UNVERIFIED"}, usage=generator.usage,
                  ...
                  max_turns=args.max_turns, max_output_tokens_per_turn=1536)
