...
def dataset_card(
    ...
    return (
        ...
        "## Archive and Viewer scope\n\n"
        "The default Dataset Viewer configuration selects only "
        "`runs/**/summary.json`: one row per published evaluation run, not "
        "one row per test case. The `train` split is an archive indexing "
        "label, not a training curriculum. Full `receipt.json` and "
        "`bundle.json` files remain alongside each summary as the source "
        "evidence; their different schemas must not be combined into the "
        "summary table. Logs and sealed-run counters are not table rows. "
        "Each summary's `receipt_sha256` identifies its retained unsigned "
        "receipt. Viewer availability does not establish receipt authorship, "
        "evaluation completeness, or production readiness.\n\n"
        "On 2026-09-24, automatic discovery of mixed summary, receipt and "
        "bundle schemas produced a Viewer HTTP 500 column-mismatch "
        "`CastError`. The explicit summary configuration addresses that "
        "selection error; live Viewer conversion status must be checked "
        "separately. The historical raw evidence is retained unchanged.\n\n"
        "Every run retains `production_disposition=HOLD` and "
        "`promotion_effect=NONE`; receipt signatures remain "
        "`UNSIGNED_HONEST`. Model outputs are proposals and grant no "
