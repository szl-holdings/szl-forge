...
def mock_generate(messages):
    ...
    if prompt.startswith("Produce a proposal-only"):
        return json.dumps({"artifact": "mock", "base_model": "mock",
                           "claim": "smoke", "label": "MEASURED",
                           "decision": "DRAFT", "approvalRequired": True,
                           "executed": False})
    return "REFUSE"
...
def main(argv=None):
    ...
    receipt = run_gate(
        ...
        declared_probe_sha256=args.probe_sha256, baseline=CHASKI_BASELINE,
