...
RECEIPT_PATH = Path(r"C:\\Users\\steph\\szl-forge\\chaski_r3\\bakeoff_canonical_r3.receipt.json")
...
def publicize_runtime(value: str | Path | None) -> str | None:
    ...
    trimmed = norm.rstrip("/")
    if trimmed.endswith(r"C:\\Users\\steph\\szl-forge\\chaski\\chaski-5050-adapter"):
        return r"C:\\Users\\steph\\szl-forge\\chaski\\chaski-5050-adapter"
    if trimmed.endswith(r"C:\\Users\\steph\\szl-forge\\chaski_r2\\chaski-r2-adapter"):
        return r"C:\\Users\\steph\\szl-forge\\chaski_r2\\chaski-r2-adapter"
...
def resolve_5050(cli: str | None) -> Path | None:
    ...
    paths.extend(
        [
            HERE / r"C:\\Users\\steph\\szl-forge\\chaski\\chaski-5050-adapter",
            Path.home() / "szl-forge" / r"C:\\Users\\steph\\szl-forge\\chaski\\chaski-5050-adapter",
        ]
    )
...
def resolve_r2(cli: str | None) -> Path | None:
    ...
    paths.extend(
        [
            ROOT / "chaski_r2" / r"C:\\Users\\steph\\szl-forge\\chaski_r2\\chaski-r2-adapter",
            Path.home()
            / "work-pr"
            / "szl-forge-r2-bf16"
            / "chaski_r2"
            / r"C:\\Users\\steph\\szl-forge\\chaski_r2\\chaski-r2-adapter",
        ]
    )
    return first_adapter(paths)
...
# ---------------------------------------------------------------------
# LOCAL R3 RECONCILIATION OVERRIDE — appended to a copied canonical file
# Generated locally; no canonical source or gate file is modified.
# ---------------------------------------------------------------------
import os as _r3_os
from pathlib import Path as _r3_Path

_R3_LOCAL_5050 = _r3_os.environ.get("CHASKI_R3_LOCAL_5050_ADAPTER", "").strip()
_R3_LOCAL_R2 = _r3_os.environ.get("CHASKI_R3_LOCAL_R2_ADAPTER", "").strip()
_R3_LOCAL_R3 = _r3_os.environ.get("CHASKI_R3_LOCAL_R3_ADAPTER", "").strip()

# Preserve original helpers before overriding. The original function is called
# first, preserving canonical defaults; explicit local paths are used only when
# the original lookup returns no existing adapter.
_r3_original_first_adapter = first_adapter

def first_adapter(paths):
    _r3_found = _r3_original_first_adapter(paths)
    if _r3_found is not None:
        return _r3_found
    for _r3_text in (_R3_LOCAL_5050, _R3_LOCAL_R2, _R3_LOCAL_R3):
        if not _r3_text:
            continue
        _r3_path = _r3_Path(_r3_text)
        if _r3_path.is_dir() and any(_r3_path.glob("*.safetensors")):
            return _r3_path
    return None
