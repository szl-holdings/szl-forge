...
# LOCAL ONLY: retain canonical runner behavior while resolving its existing
# relative gate paths from the tracked chaski directory, not chaski_r3.
HERE = Path(r"C:\\Users\\steph\\szl-forge\\chaski")
...
def publicize_runtime(value: str | Path | None) -> str | None:
    ...
    if trimmed.endswith("chaski-r3-adapter"):
        return "chaski_r3/chaski-r3-adapter"
...
def run_bakeoff(args: argparse.Namespace) -> dict[str, Any]:
    ...
    specs = [
        ...
        },
        {
            "id": "chaski-r3-local",
            "kind": "adapter",
            "hub": "SZLHOLDINGS/chaski-r3",
            "adapter": first_adapter([Path(args.chaski_r3_adapter)]) if args.chaski_r3_adapter else None,
            "does_not_overwrite": PARENT_HUB,
            "hub_id_declared_only": True,
            "local_only": True,
        },
...
def main() -> int:
    ...
    parser.add_argument("--chaski-r3-adapter", default=None)
