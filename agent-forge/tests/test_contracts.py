...
class PackageContractTests(unittest.TestCase):
    ...
    def test_schema_copies_are_semantically_identical_and_read_only(self) -> None:
        repository_schema = json.loads(
            (PACKAGE_ROOT / "schemas" / "a11oy-owned-agent-control-projection.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_exported_projection_validates_against_full_schema(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            schema = json.loads(
                (PACKAGE_ROOT / "schemas" / "a11oy-owned-agent-control-projection.schema.json")
                .read_text(encoding="utf-8")
            )
...
class ContextAlgorithmTests(unittest.TestCase):
    ...
    def test_generator_flags_conflicting_invariants(self) -> None:
        ...
        self.assertIn("a11oy_authority", evidence["consistency"]["conflicting_invariants"])


class AtomicStateTests(unittest.TestCase):
    def test_failed_schema_build_is_retryable_and_instance_identity_is_immutable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "control.sqlite3"
            with mock.patch.object(controller, "STATE_SCHEMA_SQL", "CREATE TABLE broken("):
...
class AuthorizationBindingTests(unittest.TestCase):
    ...
    def test_full_signed_envelope_is_persisted_canonically(self) -> None:
        ...
        with connect(self.primary) as connection:
            ...
            self.assertEqual(row["envelope_json"], canonical_json(envelope).decode("utf-8"))
            stored = json.loads(row["envelope_json"])
            self.assertEqual(sorted(stored["authorization"]["signatures"]), ["alice", "bob"])
            self.assertEqual(verify_audit(connection)["integrity"], "VERIFIED_LOCAL_HASH_CHAIN")
    ...
    def test_unclaimed_isolation_never_claims_supervisor_exit_or_job_cleanup(self) -> None:
...
class PersistentEvidenceTests(unittest.TestCase):
    ...
    def test_state_database_uses_strict_tables_and_immutable_target_binding(self) -> None:
    ...
    def test_parent_never_persists_unaudited_bootstrap_supervisor_identity(self) -> None:
    ...
    def test_unclaimed_reservation_times_out_without_process_or_job_claims(self) -> None:
        generated = generate_context(self.paths, TARGET, stable_context_input())
        run_id = str(uuid.uuid4())
        with connect(self.paths) as connection:
            with controller.immediate_transaction(connection):
    ...
    def test_context_trace_is_append_only(self) -> None:
        ...
        with connect(self.paths) as connection:
            with self.assertRaises(sqlite3.IntegrityError):
                connection.execute(
                    "DELETE FROM context_traces WHERE trace_id=?", (generated["trace_id"],)
                )

    def test_mutated_context_evidence_is_rejected(self) -> None:
        evidence = EnrichedContextGenerator(target_row()).generate(stable_context_input())
    ...
    def test_multiple_context_traces_are_preserved_and_latest_is_selected(self) -> None:
        ...
        self.assertEqual(read_context_trace(self.paths, trace_id=first["trace_id"]), first)
    ...
    def test_projection_is_read_only_hash_bound_and_audit_verified(self) -> None:
        ...
        self.assertEqual(projection["schema"], "a11oy/owned-agent-control-projection/v1")
        ...
        self.assertEqual(
            projection["projection_sha256"], hashlib.sha256(canonical_json(unsigned)).hexdigest()
        )
        with connect(self.paths) as connection:
            self.assertEqual(verify_audit(connection)["integrity"], "VERIFIED_LOCAL_HASH_CHAIN")
    ...
    def test_latest_context_uses_insertion_order_not_wall_clock(self) -> None:
        with connect(self.paths) as connection:
            target = dict(
                connection.execute("SELECT * FROM targets WHERE target=?", (TARGET,)).fetchone()
            )
