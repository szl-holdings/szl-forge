...
| Fall 2026 original cuts (`SZLHOLDINGS/{chaski,qantu,waman,chakana,tinku}`) | **GitHub recipes + honest job stamps** | Chaski CUTTING (Hub `adapter_model.safetensors` 25.6MB + merged 1.7GB shard + `training_receipt.json`; MEASURED train_loss ~1.7839; evals none-this-run; `6a91bb7c` ERROR after upload). Qantu/Waman SKIP. Chakana/Tinku Jobs UNKNOWN. | Files on Hub are not an eval. No Qantu/Waman trainers. KILLINCHU-EYE is a Waman alias. Hub README is not recut from this checkout. See [`chaski/README.md`](./chaski/README.md). |
