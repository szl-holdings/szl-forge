...
CUTTING. Weights AVAILABLE on Hub. Evals remain none-this-run.
Train loss is not an eval. Do not invent 5/5. Not ROADMAP.
Do not pin serve. Do not recut Hub from this checkout (QURI owns Hub copy).
6a91bf10 COMPLETED, weights AVAILABLE. 6a91bb7c stays ERROR.
"""
...
# Attempt 5 (6a91bf10) COMPLETED, weights AVAILABLE. Evals none-this-run.
JOBS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "status": "COMPLETED",
        "detail": (
            "report_to=none. weights AVAILABLE. Files on repo as of "
            f"{HUB_TENSORS_OBSERVED_AT}: "
            + ", ".join(HUB_TENSORS)
            + ". Evals none-this-run. Train loss is not an eval. "
            "Do not invent 5/5. Not ROADMAP."
        ),
        "url": LIVE_JOB_URL,
        "report_to": "none",
        "weights": "AVAILABLE",
...
def cutting_receipt(*, live: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        "claim_boundary": (
            ...
            "Train loss is not an eval. CUTTING. 6a91bf10 COMPLETED, "
...
def status_main() -> int:
    ...
    print(f"[chaski] 6a91bf10 COMPLETED weights=AVAILABLE: {LIVE_JOB_URL}")
    for job in JOBS:
