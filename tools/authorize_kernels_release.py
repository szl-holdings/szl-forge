#!/usr/bin/env python3
"""Authorize one exact kernels release before any publisher secret is exposed."""
...
def github_get(path: str, token: str | None) -> dict[str, Any]:
    headers = {
        ...
        "User-Agent": "szl-forge-kernels-release-gateway",
    ...
    *,
    ...
    if source_revision != source_main:
        raise AuthorizationError(
            f"source revision is not current protected main "
            f"(requested {source_revision}, protected main {source_main})"
        )
    if publisher_revision != publisher_main:
        raise AuthorizationError(
            f"publisher revision is not current protected main "
    ...
    if verification.get("verified") is not True:
        raise AuthorizationError(
            f"source commit signature is not verified: "
    ...
    *,
    ...
    while True:
        try:
            ...
            if (
                "pending" not in str(error)
                and "missing" not in str(error)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/kernels-release-authorization.json"),
    )
