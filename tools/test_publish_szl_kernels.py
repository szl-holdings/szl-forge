...
class PublishSzlKernelsTests(unittest.TestCase):
    ...
    def test_runtime_verifier_normalizes_exception_type_grammar(self) -> None:
        punctuated_error = type("Bad-Type!", (Exception,), {})()
        unicode_error = type("\u00c9rror", (Exception,), {})()
        self.assertEqual(
            runtime_verifier._normalized_error_type(punctuated_error),
            "Bad_Type_",
        )
        self.assertEqual(
            runtime_verifier._normalized_error_type(unicode_error),
            "_rror",
        )

    def test_runtime_verifier_preserves_system_exit_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            output = Path(temporary) / "evidence.json"
            with patch.object(
                runtime_verifier,
                "verify_stable_kernel_runtime",
                side_effect=SystemExit("stable runtime requested exit"),
            ):
                result = runtime_verifier.main(
                    ["--revision", "2" * 40, "--output", str(output)]
                )
        ...
        self.assertEqual(evidence["error"], "stable runtime requested exit")
