...
class Fall2026AlignmentTests(unittest.TestCase):
    ...
    def test_chaski_5050_is_new_id_not_live_chaski(self) -> None:
        ...
        self.assertIn('ALLOWED_HUB = "SZLHOLDINGS/chaski-5050"', text)
        self.assertIn("HUB = os.environ.get(\"HUB_MODEL_ID\", ALLOWED_HUB)", text)
        self.assertIn('CANONICAL_BASE = "Qwen/Qwen3.5-0.8B"', text)
        ...
        self.assertIn("WARMUP_STEPS = 6", text)
        self.assertIn("QLoRA forbidden", text)
        self.assertNotIn("LORA_ALPHA = 32", text)
        self.assertNotIn("6a91bf10", text)
        ...
        self.assertIn("QLoRA forbidden", card)
        self.assertIn("Not an HF Job", card)
        self.assertIn("REPORTED owner-metal", card)
        self.assertIn("publication_eligible: false", card)
        self.assertNotIn("6a91bf10", eval_script.read_text(encoding="utf-8"))
        self.assertNotIn("6a91bf10", serve.read_text(encoding="utf-8"))
        completed = subprocess.run(
