...
"""Publish the reviewed KHIPU-R2 model card as an exact-source Hub commit.

Only ``README.md`` and ``holo-banner.svg`` are writable. Model weights,
...
SOURCE_FILES = {
    "README.md": ROOT / "khipu-r2" / "card" / "README.md",
    "holo-banner.svg": ROOT / "khipu-r2" / "card" / "holo-banner.svg",
}
CANONICAL_EVIDENCE = ROOT / "khipu_r2" / "README.md"
...
REQUIRED_CARD_BOUNDARIES = (
    ...
    "Abstain is MEASURED 3/6, not a pass.",
...
REQUIRED_EVIDENCE_BOUNDARIES = (
    "adapter **147.8MB AVAILABLE**",
    "abstain **MEASURED 3/6**",
    "grounding **5/5**",
    "plan **11/11**",
    "publication_eligible: false",
    "No Hub PUT.",
...
def load_canonical_evidence() -> bytes:
    if not CANONICAL_EVIDENCE.is_file() or CANONICAL_EVIDENCE.is_symlink():
        raise PublicationError("canonical KHIPU-R2 evidence is missing or unsafe")
    return CANONICAL_EVIDENCE.read_bytes()
...
def validate_assets(
    ...
    evidence = load_canonical_evidence() if evidence is None else evidence
    try:
        card = assets["README.md"].decode("utf-8")
        banner = assets["holo-banner.svg"].decode("utf-8")
        evidence_text = evidence.decode("utf-8")
    except UnicodeDecodeError as error:
        raise PublicationError("card assets and canonical evidence must be UTF-8") from error
    ...
    for boundary in REQUIRED_EVIDENCE_BOUNDARIES:
    ...
    *,
    ...
    commit = publish_with_bounded_retry(
        lambda: api.create_commit(
            ...
            commit_message=f"docs: publish KHIPU-R2 card from szl-forge@{source_revision}",
            commit_description=(
                "Exact-source documentation publication. The adapter and evaluation "
                "artifacts are unchanged; abstention remains MEASURED 3/6 and not a "
                "pass; publication_eligible and autonomy_eligible remain false."
            ),
        )
    )
...
def build_report(
    *,
    ...
    return {
        "schema": "szl.hf.khipu-r2-card-publication/v1",
        ...
        "source": {
            ...
            "evidence": {
                "path": str(CANONICAL_EVIDENCE.relative_to(ROOT)),
...
def main(argv: list[str] | None = None) -> int:
    ...
    source_evidence = validate_assets(assets, evidence)
