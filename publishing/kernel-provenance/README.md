...
This is a metadata-only repair for eight existing `SZLHOLDINGS` **model** repositories. It is not a kernel deployment or model-training release. The same-named Hub kernel repositories are outside the write set.
...
`snapshots.json` contains public September 19, 2026 exact-revision metadata, tree records, request receipts, and the original provenance bytes. The old documents are preserved both as parsed historical evidence and verbatim base64 bytes in each replacement. Their historical claims, metrics, dependencies, and source assertions are **not** adopted as current verification.
